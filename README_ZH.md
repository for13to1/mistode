...
- **🐍 Python 支持 (v3.14+)**:
  ...
  - **文档字符串隐藏**: 将文档字符串替换为最小占位符（从嵌入布局数据中无损还原）。
...
- **🔄 零损还原 (Zero-Loss Restoration)**:
  - **分布式布局数据**: 布局数据（空白、注释和字符串内容）被压缩并作为注释分块分布在整个混淆文件中（默认 base64，使用 `--password` 时加密）。
  - **嵌入式元数据**: 标识符映射直接嵌入在文件页脚。**恢复文件不需要密钥文件。**
  - **比特级完美还原**: 还原原始代码的每一个字节，包括注释、格式和空行。

- **⚙️ 现代工具链**:
  - **配置文件**: 通过 `pyproject.toml` 设置全局默认值。
  - **详细统计**: `--stats` 标志提供标识符计数、保留名计数和文件大小分析。
...
1. **分布式布局数据 (首选)**:
    布局数据（空白、注释和字符串内容）被压缩并作为注释（例如 `#@mistode:chunk:...`）注入到整个文件中。这允许**100% 比特级完美还原**。

2. **嵌入式映射 (次选)**:
    重命名映射被压缩并嵌入在文件页脚（`#@mistode:metadata:`）。如果布局数据损坏，这允许功能性还原。
